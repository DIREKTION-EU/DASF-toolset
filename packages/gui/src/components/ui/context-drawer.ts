  ...
  return {
    ...
    view: ({ attrs }) => {
      ...
      const tag = (text: string, color?: string) =>
        m(
          "span.context-drawer-tag",
          { style: color ? `background:${color}20; color:${color}` : "" },
          text,
        );
        ...
        if (drawerItem.type === "capability") {
            ...
            content = [
              ...
              section(`${t("drawer_solutions")} (${capSolutions.length})`, [
                ...
                m(
                  ...
                  link(t("drawer_go_to_solutions"), () =>
                    navigate(Pages.SOLUTIONS),
                  ),
                ),
              ]),
          ...
          if (hazard) {
            const linkedCaps = capabilities.filter((c) =>
              (c.hazardIds || []).includes(hazard.id),
            );
            const withGaps = linkedCaps.filter(
              (c) => c.gaps && c.gaps.length > 0,
            );
            const withoutGaps = linkedCaps.filter(
              (c) => !c.gaps || c.gaps.length === 0,
            );
