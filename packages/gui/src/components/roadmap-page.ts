...
export const RoadmapPage: MeiosisComponent = () => {
  return {
    oninit: ({ attrs }) => {
      ...
      const { data = {} } = catModel;
      if ((!data.roadmapItems || data.roadmapItems.length === 0) && data.solutions && data.solutions.length > 0) {
    ...
    view: ({ attrs }) => {
      ...
      const priorityColors: Record<string, string> = { low: '#4caf50', medium: '#ff9800', high: '#f44336' };
      const solutionsNotInRoadmap = solutions.filter(s => !roadmapItems.find(r => r.solutionId === s.id));
      ...
      return m(".roadmap.page", [
        ...
        solutionsNotInRoadmap.length > 0 && m('.row', [
          m('.col.s12', m(FlatButton, {
            iconName: 'playlist_add',
            label: t('roadmap_add_solutions', { n: solutionsNotInRoadmap.length }),
            onclick: () => {
              const newItems: IRoadmapItem[] = solutionsNotInRoadmap.map(s => ({
                solutionId: s.id,
                priority: 'medium' as const,
              }));
                ...
                return m('tr', { key: item.solutionId }, [
                  ...
                  m('td', [
                    m('select.browser-default', {
                      ...
                      onchange: (e: Event) => {
                        item.priority = (e.target as HTMLSelectElement).value as 'low' | 'medium' | 'high';
                        actions.saveModel(attrs, catModel);
                      },
                    }, [
                      m('option', { value: 'low' }, t('priority_low')),
                      m('option', { value: 'medium' }, t('priority_medium')),
                      m('option', { value: 'high' }, t('priority_high')),
                    ]),
                  ]),
                  m('td', [
                    m('input[type=date]', {
                      ...
                      onchange: (e: Event) => {
                        item.targetDate = (e.target as HTMLInputElement).value;
                        actions.saveModel(attrs, catModel);
                      },
                    }),
                  ]),
                  m('td', [
                    m(TextInput, {
                      ...
                      initialValue: item.commitment || '',
                      placeholder: t('proj_sum'),
                      onchange: (v) => { item.commitment = v; actions.saveModel(attrs, catModel); },
                    }),
                  ]),
                  m('td', m(FlatButton, {
                    iconName: 'delete',
                    ...
                    onclick: () => {
                      data.roadmapItems = roadmapItems.filter(r => r.solutionId !== item.solutionId);
                      actions.saveModel(attrs, catModel);
                    },
                  })),
                ]);
        ...
        // Visual timeline
        roadmapItems.length > 0 && roadmapItems.some(r => r.targetDate) && m('.row', { style: 'margin-top: 30px;' }, [
            ...
            m('.timeline-container',
              roadmapItems
                ...
                .map(item => {
                  const sol = getSolution(item.solutionId);
                  if (!sol) return null;
                  return m('.timeline-item.card', {
                    key: item.solutionId,
                    style: `border-left: 4px solid ${priorityColors[item.priority || 'medium']};`,
                  }, m('.card-content', [
                    m('span.card-title', sol.label),
                    m('p', [
                      m('strong', t('start_time') + ': '), item.targetDate,
                      ' | ',
                      m('strong', t('importance') + ': '),
                      m('span', { style: `color: ${priorityColors[item.priority || 'medium']}` }, item.priority || 'medium'),
                    ]),
